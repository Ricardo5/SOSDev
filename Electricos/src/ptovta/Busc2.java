...
/*Clase para controlar la búsqueda genérica 2*/
public class Busc2 extends javax.swing.JDialog
{
    /*Contiene el color original del botón*/
    private final java.awt.Color        colOri;
    
    /*Declara variables de instancia*/
    private static  Busc2               obj = null; 
    private final int                   iEn;
    private final String                sBusc;
    private final JTextField            jTFCamp;
    private final JTextField            jTFCamp2;   
    private final JTextField            jTFCamp4;    
    
            
    
    /*Constructor sin argumentos*/
    public Busc2(JFrame jFram, String sBusc1, int iE, JTextField jCamp ,JTextField jCamp2, JTextField jCamp4) 
    {           
        ...
        this.setTitle("Busqueda general 2, Usuario: <" + Login.sUsrG + "> " + Login.sFLog);        
    ...
    public static Busc2 getObj(JFrame jFram, String sBusc, int iEn, JTextField jCampo, JTextField jCampo2, JTextField jCampo3, JTextField jCampo4)
    {
        /*Si es null entonces crea una nueva instancia*/
        if(obj==null)
            obj = new Busc2(jFram, sBusc, iEn, jCampo, jCampo2, jCampo4);
   ...
   /*Obtiene todos los registros y cargalos en la tabla*/
    private void CargProds(String sBusc)
    {
        ...
        try
        {
            sQ = "SELECT compue, prod, descrip, alma, exist, unid FROM prods WHERE 1=1 AND (CASE WHEN '" + sBusc + "' = '' THEN prod = prod ELSE prod LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN descrip = descrip ELSE descrip LIKE('%" + sBusc + "%') END) ORDER BY prod";                        
            ...
            while(rs.next())
            {
                ...
                /*Agregalo a la tabla*/
                Object nu[]     = {rs.getString("prod"), rs.getString("descrip"), sExist + " " + rs.getString("unid")};
                tm.addRow(nu);                               
            }                        
        }
    ...
    private void jBCargActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBCargActionPerformed
        ...
        jTFCamp4.setText    (jTab.getValueAt(jTab.getSelectedRow(), 2).toString());                    
        
        /*Coloca los cursores al principio*/
        jTFCamp.setCaretPosition(0);
        jTFCamp2.setCaretPosition(0);
        jTFCamp4.setCaretPosition(0);                    
