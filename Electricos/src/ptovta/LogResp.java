...
/*Clase para ver el log de los respaldos*/
public class LogResp extends javax.swing.JFrame 
{
    ...
    private String          sRespOri;
    private String          sMyDeOri;
    private String          sMyAOri;
    private String          sArchDOri;
    private String          sArchAOri;
    private String          sFechOri;
    private String          sErrOri;
    ...
    {                
        ...
        this.setTitle("Log respaldos, Usuario: <" + Login.sUsrG + "> " + Login.sFLog);        
        ...
        /*Carga todos los logs de los resplados en la tabla*/
        vCargReg();
            ...
            public void propertyChange(PropertyChangeEvent event) 
            {
                ...
                if("tableCellEditor".equals(pr)) 
                {
                    ...
                    if(iContCellEd==1)
                    {
                        ...
                        sRespOri        = jTab.getValueAt(jTab.getSelectedRow(), 2).toString();
                        sMyDeOri        = jTab.getValueAt(jTab.getSelectedRow(), 3).toString();
                        sMyAOri         = jTab.getValueAt(jTab.getSelectedRow(), 4).toString();
                        sArchDOri       = jTab.getValueAt(jTab.getSelectedRow(), 5).toString();
                        sArchAOri       = jTab.getValueAt(jTab.getSelectedRow(), 6).toString();
                        sFechOri        = jTab.getValueAt(jTab.getSelectedRow(), 7).toString();
                        sErrOri         = jTab.getValueAt(jTab.getSelectedRow(), 8).toString();
                    ...
                    else
                    {
                        ...
                        jTab.setValueAt(sRespOri,       jTab.getSelectedRow(), 2);                        
                        jTab.setValueAt(sMyDeOri,       jTab.getSelectedRow(), 3);                        
                        jTab.setValueAt(sMyAOri,        jTab.getSelectedRow(), 4);                        
                        jTab.setValueAt(sArchDOri,      jTab.getSelectedRow(), 5);                        
                        jTab.setValueAt(sArchAOri,      jTab.getSelectedRow(), 6);                                                
                        jTab.setValueAt(sFechOri,       jTab.getSelectedRow(), 7);                        
                        jTab.setValueAt(sErrOri,        jTab.getSelectedRow(), 8);                        
    ...
    }/*Fin de public LogColo() */
        
    
    /*Carga todos los logs de los respaldos en la tabla*/
    ...
    {        
        ...
        /*Obtiene todos los logs de los respaldos de la base de datos*/
        try
        {
            sQ = "SELECT CASE WHEN tip = 0 THEN 'I' ELSE 'E' END AS tip, CASE WHEN `return` = -1 THEN '' ELSE  `return` END AS 'RET', pathdemysq, pathamysq, pathde, patha, resplog.FALT, msj, resplog.SUCU, resplog.NOCAJ, resplog.ESTAC, estacs.NOM FROM resplog LEFT OUTER JOIN estacs ON estacs.ESTAC = resplog.ESTAC ORDER BY resplog.ID_ID DESC";
            ...
            while(rs.next())
            {
                ...
                Object nu[]             = {iContFi, rs.getString("tip"), rs.getString("RET"), rs.getString("resplog.PATHDEMYSQ"), rs.getString("resplog.PATHAMYSQ"), rs.getString("resplog.PATHDE"), rs.getString("resplog.PATHA"), rs.getString("resplog.FALT"), rs.getString("resplog.MSJ"), rs.getString("resplog.SUCU"), rs.getString("resplog.NOCAJ"), rs.getString("resplog.ESTAC"), rs.getString("estacs.NOM")};
                te.addRow(nu);                
    ...
    private void initComponents() {
        ...
        jTab.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "No.", "Tipo", "Respuesta", "MySQL De", "MySQL A", "Archivos De", "Archivos A", "Fecha", "Error", "Sucursal", "Caja", "Usuario", "Nombre Usuario"
            }
        ) {
        ...
        jBRep.setToolTipText("Ver Reporte de Colores (Ctrl+R)");
        ...
        layout.setHorizontalGroup(
            ...
            .addComponent(jP1, javax.swing.GroupLayout.PREFERRED_SIZE, 614, javax.swing.GroupLayout.PREFERRED_SIZE)
        );
    ...
    private void jBRepActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBRepActionPerformed
        ...
        /*Muestra la forma para reportear los logs de respaldos*/
        RepResp c = new RepResp();
        c.setVisible(true);
    ...
    private void jBBuscActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBBuscActionPerformed
        ...
        try
        {            
            sQ = "SELECT  CASE WHEN tip = 0 THEN 'I' ELSE 'E' END AS tip, CASE WHEN `return` = -1 THEN '' ELSE  `return` END AS 'RET', pathdemysq, pathamysq, pathde, patha, resplog.FALT, msj, resplog.SUCU, resplog.NOCAJ, resplog.ESTAC, estacs.NOM  FROM resplog LEFT OUTER JOIN estacs ON estacs.ESTAC = resplog.ESTAC  WHERE resplog.ESTAC LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR resplog.SUCU LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR estacs.NOCAJ LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR resplog.FALT LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR resplog.ESTAC LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR resplog.SUCU LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR resplog.NOCAJ LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR estacs.NOM LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') ORDER BY resplog.ID_ID DESC";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
            /*Si hay datos*/
            while(rs.next())
    ...
    /*Cuando sale el mouse del botón de búscar*/
