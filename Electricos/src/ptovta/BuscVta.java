...
public class BuscVta extends javax.swing.JFrame
{
    ...
    public BuscVta(JFrame jFram, String sBu, JTextField jCamp1, String sTip, javax.swing.JTable jTa, String sClie, int iContF) 
    {           
        ...
        /*Obtiene el código del cliente de quién se mostraran los documentos*/
        sCli        = sClie;
        ...
        this.setTitle("Busqueda ventas, Usuario: <" + Login.sUsrG + "> " + Login.sFLog);        
        ...
        /*Si el tipo es de facturas entonces muestra las facturas*/
    ...
    private void vCargFac(String sBusc)
    {        
        ...
        try
        {
            sQ = "SELECT nom, vta, norefer, noser, tipdoc, vtas.CODEMP, vtas.SER, vtas.METPAG, vtas.CTA, femi, subtot, impue, tot, vtas.ESTAD, motiv, vtas.OBSERV, CASE WHEN timbr THEN 'Si' ELSE 'No' END AS timbr, vtas.SUCU, vtas.NOCAJ, vtas.ESTAC, fvenc FROM vtas LEFT OUTER JOIN emps ON CONCAT_WS('', emps.SER, emps.CODEMP) = vtas.CODEMP WHERE tipdoc = 'FAC'  AND notcred = '' AND vtas.CODEMP = '" + sCli + "' AND (CASE WHEN '" + sBusc + "' = '' THEN norefer = norefer ELSE norefer LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN noser = noser ELSE CONCAT_WS('', noser, norefer) LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN noser = noser ELSE noser LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN tipdoc = tipdoc ELSE tipdoc LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN vtas.CODEMP = vtas.CODEMP ELSE vtas.CODEMP LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN vtas.SER = vtas.SER ELSE vtas.SER LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN vtas.METPAG = vtas.METPAG ELSE vtas.METPAG LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN vtas.CTA = vtas.CTA ELSE vtas.CTA LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN femi = femi ELSE femi LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN subtot = subtot ELSE subtot LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN impue = impue ELSE impue LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN tot = tot ELSE tot LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN vtas.ESTAD = vtas.ESTAD ELSE vtas.ESTAD LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN motiv = motiv ELSE motiv LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN vtas.OBSERV = vtas.OBSERV ELSE vtas.OBSERV LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN vtas.SUCU = vtas.SUCU ELSE vtas.SUCU LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN vtas.NOCAJ = vtas.NOCAJ ELSE vtas.NOCAJ LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN vtas.ESTAC = vtas.ESTAC ELSE vtas.ESTAC LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN fvenc = fvenc ELSE fvenc LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN nom = nom ELSE nom LIKE('%" + sBusc + "%') END) ORDER BY vta DESC";                        
            ...
            {
                /*Agregalo a la tabla*/
                Object nu[]     = {rs.getString("vta"), rs.getString("norefer"), rs.getString("noser"), rs.getString("tipdoc"), rs.getString("codemp"), rs.getString("ser"), rs.getString("metpag"), rs.getString("cta"), rs.getString("femi"), rs.getString("fvenc"), rs.getString("subtot"), rs.getString("impue"), rs.getString("tot"), rs.getString("vtas.ESTAD"), rs.getString("motiv"), rs.getString("observ"), rs.getString("timbr"), rs.getString("sucu"), rs.getString("nocaj"), rs.getString("estac")};
                tm.addRow(nu);                                
            }                        
        }
    ...
    }/*Fin de private void vCargFac(String sBusc, int iEn)*/
    ...
    private void vCargNot(String sBusc)
    {
        ...
        try
        {
            sQ = "SELECT nom, vta, norefer, noser, tipdoc, vtas.CODEMP, vtas.SER, vtas.METPAG, vtas.CTA, femi, subtot, impue, tot, vtas.ESTAD, motiv, vtas.OBSERV, CASE WHEN timbr THEN 'Si' ELSE 'No' END AS timbr, vtas.SUCU, vtas.NOCAJ, vtas.ESTAC, fvenc FROM vtas LEFT OUTER JOIN emps ON CONCAT_WS('', emps.SER, emps.CODEMP) = vtas.CODEMP WHERE tipdoc = 'NOTC'  AND vtas.ESTAD = 'PE' AND vtas.CODEMP = '" + sCli + "' AND (CASE WHEN '" + sBusc + "' = '' THEN norefer = norefer ELSE norefer LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN noser = noser ELSE CONCAT_WS('', noser, norefer) LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN noser = noser ELSE noser LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN tipdoc = tipdoc ELSE tipdoc LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN vtas.CODEMP = vtas.CODEMP ELSE vtas.CODEMP LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN vtas.SER = vtas.SER ELSE vtas.SER LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN vtas.METPAG = vtas.METPAG ELSE vtas.METPAG LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN vtas.CTA = vtas.CTA ELSE vtas.CTA LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN femi = femi ELSE femi LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN subtot = subtot ELSE subtot LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN impue = impue ELSE impue LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN tot = tot ELSE tot LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN vtas.ESTAD = vtas.ESTAD ELSE vtas.ESTAD LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN motiv = motiv ELSE motiv LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN vtas.OBSERV = vtas.OBSERV ELSE vtas.OBSERV LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN vtas.SUCU = vtas.SUCU ELSE vtas.SUCU LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN vtas.NOCAJ = vtas.NOCAJ ELSE vtas.NOCAJ LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN vtas.ESTAC = vtas.ESTAC ELSE vtas.ESTAC LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN fvenc = fvenc ELSE fvenc LIKE('%" + sBusc + "%') END OR CASE WHEN '" + sBusc + "' = '' THEN nom = nom ELSE nom LIKE('%" + sBusc + "%') END) ORDER BY vta DESC";                        
            st = con.createStatement();
            rs = st.executeQuery(sQ);
            /*Si hay datos*/
            while(rs.next())
    ...
    private void initComponents() {
        ...
        jTab.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Venta", "Folio", "Serie", "Tipo Documento", "Cod.Cliente", "Serie Cliente", "Método Pago", "Cuenta", "Fecha", "Fecha Vencimiento", "SubTotal", "Impuesto", "Total", "Estado", "Motivo", "Observaciones", "Timbrada", "Sucursal", "Caja", "Usuario"
            }
        ) {
    ...
    private void jBCargActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBCargActionPerformed
        ...
        if(sTipDoc.compareTo("fac")==0)                   
            jTFCamp.setText     (jTab.getValueAt(jTab.getSelectedRow(), 2).toString() + jTab.getValueAt(jTab.getSelectedRow(), 1).toString());       
        ...
        {                               
            ...
            {   
                ...
                Object nu[]             = {iContFi, jTab.getValueAt(iSel[x], 2).toString() + jTab.getValueAt(iSel[x], 1).toString(), "0", "", "", "1", "APLICA NOTA DE CRÉDITO", "", "", "", sImpo, "", "", "", "", "", "", "", "", "", "", "$0.00", "", "", "", "0", ""};
                tm.addRow(nu);
            }               
        }
