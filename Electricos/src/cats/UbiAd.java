...
/*Clase para controlar las ubicaciones adicionales*/
public class UbiAd extends javax.swing.JFrame 
{
    /*Declara variables originales*/        
    private String          sUbiOriG;
    ...
    {                
        ...
        /*Pon el foco del teclado en el campo del código de la ubicación adicional*/
        jTCod.grabFocus();
        ...
        /*Obtiene todas las ubicaciones adicionales de la base de datos y cargalos en la tabla*/
        vCargUbi();
            ...
            {                                
                ...
                if("tableCellEditor".equals(pr)) 
                {
                    ...
                    {                                                
                        ...
                        sUbiOriG            = jTab.getValueAt(jTab.getSelectedRow(), 1).toString();                                                                                                          
                    ...
                    {
                        /*Reinicia el contador del celleditor*/
                        iContCellEd         = 1;
                        
                        /*Coloca los datos originales en su lugar*/
                        jTab.setValueAt(sUbiOriG, jTab.getSelectedRow(), 1);
                        
                        /*Obtiene la descripción nueva*/                                                
                        ...
                        {
                            /*La descripción sera la original*/
                            sDescrip = sDescripOriG;
                            
                            /*Coloca nuevamente la descripción en el control*/
                        ...
                        /*Actualiza en la base de datos la ubicación*/
                        try 
                        {                                                        
                            sQ = "UPDATE ubiad SET "
                        ...
                        {            
                            sQ = "INSERT INTO logubiad(cod,                                                                                     descrip,                            accio,                  estac,                          sucu,                           nocaj,                  falt) " + 
                                          "VALUES('" + jTab.getValueAt(jTab.getSelectedRow(), 1).toString().replace("'", "''") + "', '" +       sDescrip.replace("'", "''") + "',   'MODIFICAR',    '" +    Login.sUsrG + "',   '" +    Star.sSucu + "',  '" +    Star.sNoCaj + "', now())";                                
                            st = con.createStatement();
                            st.executeUpdate(sQ);
                        }
                        catch(SQLException expnSQL) 
    ...
    private void initComponents() {
        ...
        jBDel.setToolTipText("Borrar Ubicación(es) (Ctrl+SUPR)");
        ...
        jBNew.setText("Nuevo");
        jBNew.setToolTipText("Nuevo Ubicación (Ctrl+N)");
    ...
    /*Obtiene todas las ubicaciones de la base de datos y cargalas en la tabla*/
    private void vCargUbi()
    {
        //Abre la base de datos                             
        ...
        /*Trae todas las ubicaciones de la base de datos y cargalos en la tabla*/
        try
        {
            sQ = "SELECT cod, descrip FROM ubiad WHERE cod != '-'";
            st = con.createStatement();
    ...
    private void jBDelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBDelActionPerformed
        ...
        {
            /*Mensajea*/
            JOptionPane.showMessageDialog(null, "No has seleccionado una ubicación de la lista para borrar.", "Borrar", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd)));
        ...
        Object[] op = {"Si","No"};
        int iRes    = JOptionPane.showOptionDialog(this, "¿Seguro que quiere borrar la(s) ubicación(es)?", "Borrar", JOptionPane.YES_NO_OPTION,  JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconDu)), op, op[0]);
        ...
        {
            //Comprueba si esta ubicación existe en algún producto        
            try
            {
                sQ = "SELECT codubi FROM prods WHERE prod = '" + jTab.getValueAt(iSel[x], 1).toString().trim() + "'";                        
                st = con.createStatement();
                rs = st.executeQuery(sQ);
                /*Si hay datos, entonces si esta asociada esta ubicación a algún producto*/
                if(rs.next())
                {
                    /*Mensajea y continua*/
                    JOptionPane.showMessageDialog(null, "Ya esta esta ubicación: " + jTab.getValueAt(iSel[x], 1).toString() + " asignada a algún producto, no se puede eliminar.", "Ubicaciones", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd)));                    
                    continue;
                }
            }
            catch(SQLException expnSQL)
            ...
            try 
            {                
                sQ = "DELETE FROM ubiad WHERE cod = '" + jTab.getValueAt(iSel[x], 1).toString() + "'";                                            
                st = con.createStatement();
                st.executeUpdate(sQ);
             }
             catch(SQLException expnSQL) 
            ...
            {            
                sQ = "INSERT INTO logubiad(cod,                                                                     descrip,                                                            accio,                  estac,                          sucu,                           nocaj,                      falt) " + 
                              "VALUES('" + jTab.getValueAt(iSel[x], 2).toString().replace("'", "''") + "', '" +     jTab.getValueAt(iSel[x], 2).toString().replace("'", "''") + "',     'BORRAR',       '" +    Login.sUsrG + "',   '" +    Star.sSucu + "',  '" +    Star.sNoCaj + "',     now())";                                
                st = con.createStatement();
                st.executeUpdate(sQ);
            }
            catch(SQLException expnSQL) 
        ...
        /*Mensajea de éxito*/
        JOptionPane.showMessageDialog(null, "Ubicación(es) borrada(s) con éxito.", "Ubicaciones", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd))); 
    ...
    /*Cuando se presiona una tecla en el campo del control de ubicación adicional*/
    ...
    /*Cuando se gana el foco del teclado en el control del código de la ubicación*/
    ...
    /*Cuando se gana el foco del teclado en el campo de edición de texto de descripción de ubicación adicional*/
    ...
    /*Cuando se pierde el foco del teclado en el campo del código de la ubicación adicional*/
    ...
    private void jTDescripFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_jTDescripFocusLost
        ...
        /*Si el campo excede la cantidad de carácteres permitidos recortalo*/
    ...
    private void jBNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBNewActionPerformed
        ...
        /*Si hay cadena vacia en el campo de descripción entonces*/
        if(jTDescrip.getText().compareTo("")==0)
        {
            ...
            /*Mensajea*/
            JOptionPane.showMessageDialog(null, "El campo de descripción de peso esta vacio.", "Campo vacio", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd)));
        ...
        Object[] op = {"Si","No"};
        if((JOptionPane.showOptionDialog(this, "¿Seguro que están bien los datos?", "Agregar ubicación", JOptionPane.YES_NO_OPTION,  JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconDu)), op, op[0])) == JOptionPane.NO_OPTION)
            return;
        ...
        //Si ya existe la ubicación entonces
        if(iRes==1)
        {
            //Cierra la base de datos
            if(Star.iCierrBas(con)==-1)
        ...
        {            
            sQ = "INSERT INTO logubiad(cod,                                                                                         descrip,                                                            accio,                  estac,                          sucu,                       nocaj,                  falt) " + 
                          "VALUES('" + jTCod.getText().replace(" ", "").replace(",", "").replace("'", "''").trim() + "', '" +       jTDescrip.getText().replace("'", "''").replace(",", "") + "',       'AGREGAR',      '" +    Login.sUsrG + "',   '" +    Star.sSucu + "','" +  Star.sNoCaj + "', now())";                                
            st = con.createStatement();
            st.executeUpdate(sQ);
        }
        catch(SQLException expnSQL) 
        ...
        try 
        {            
            sQ = "INSERT INTO ubiad(cod,                                                                    descrip,                                        falt,       estac,                                      sucu,                                         nocaj) " + 
                       "VALUES('" + jTCod.getText().replace(" ", "").trim().replace("'", "''") + "','" +    jTDescrip.getText().replace("'", "''") + "',    now(), '" + Login.sUsrG.replace("'", "''") + "','" +    Star.sSucu.replace("'", "''") + "','" +       Star.sNoCaj.replace("'", "''") + "')";                                
            st = con.createStatement();
            st.executeUpdate(sQ);
         }
         catch(SQLException expnSQL) 
        ...
        /*Pon el foco del teclado en el campo del código de la ubicación*/
        jTCod.grabFocus();
        ...
        /*Mensajea de éxito*/
        JOptionPane.showMessageDialog(null, "Ubicación: " + jTDescrip.getText() + " agregada con éxito.", "Exito al agregar", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd)));                    
         
    }//GEN-LAST:event_jBNewActionPerformed

    
    /*Cuando se tipea una tecla en el campo de código de la ubicación adicional*/
    ...
    private void vCargT()
    {
        ...
        /*Obtiene todos las ubicaciones de la base de datos y cargalos en la tabla*/
        vCargUbi();
    ...
    private void jBBuscActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBBuscActionPerformed
        ...
        /*Obtiene de la base de datos todas las ubicaciones*/        
        try
        {                  
            sQ = "SELECT * FROM ubiad WHERE cod LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR descrip LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR falt LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR fmod LIKE('%" + jTBusc.getText().replace(" ", "%") + "%')";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
            /*Si hay datos*/
            while(rs.next())
