/*Alan Isaim Flores Diaz 3/12/2014 1:21pm*/
//Paquete
package cats;
...
/*Clase para controlar las clasificaciones de las clasemp*/
public class ClasCli extends javax.swing.JFrame 
{
    ...
    public ClasCli() 
    {
        ...
        /*Pon el foco del teclado en el campo de edición de código de la clasificación*/
        jTCod.grabFocus();
        
        /*Establece el tamaño de las columnas de la tabla de clasificación*/
        ...
        /*Obtén de la base de datos las clasificaciones existentes*/
        vCargClas();
            
        /*Crea el listener para la tabla de clasificaciones, para cuando se modifique una celda guardarlo en la base de datos el cambio*/
        ...
        /*Establece el listener para la tabla de clasificaciones*/
        jTab.addPropertyChangeListener(pro);
                
    }/*Fin de public CalsiClient() */

        
    /*Obtén de la base de datos las clasificaciones y cargalas en la tabla*/
    private void vCargClas()
    {
        //Abre la base de datos                             
        Connection  con = Star.conAbrBas(true, false);
        ...
        /*Trae todas las clasificaciones de la base de datos y cargalos en la tabla*/
        try
        {
            sQ = "SELECT cod, descrip FROM clasemp WHERE cod <> '-'";
            st = con.createStatement();
            ...
            {
                /*Agregalo a la tabla*/
                Object nu[]         = {iContFi, rs.getString("cod"), rs.getString("descrip")};
                te.addRow(nu);
    ...
    private void initComponents() {
        ...
        jBDel.setToolTipText("Borrar clasificación(es) (Ctrl+SUPR)");
        ...
        jBNew.setText("Nuevo");
        jBNew.setToolTipText("Nueva clasificacion (Ctrl+N)");
        ...
        jP1.add(jLAyu, new org.netbeans.lib.awtextra.AbsoluteConstraints(486, 340, 130, 10));
    ...
    private void jBDelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBDelActionPerformed
        ...
        {
            /*Mensajea*/
            JOptionPane.showMessageDialog(null, "No has seleccionado una clasificación de la lista para borrar.", "Borrar Clasificación", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd)));
        ...
        Object[] op = {"Si","No"};
        int iRes    = JOptionPane.showOptionDialog(this, "¿Seguro que quiere borrar la(s) clasificación(es)?", "Borrar Clasificación", JOptionPane.YES_NO_OPTION,  JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconDu)), op, op[0]);
        ...
        {
            /*Comprueba si esta clasificación existe en algúna empresa*/        
            try
            {
                sQ = "SELECT codemp FROM emps WHERE codclas = '" + jTab.getValueAt(iSel[x], 1).toString().trim() + "'";                        
                st = con.createStatement();
                rs = st.executeQuery(sQ);
                /*Si hay datos, entonces si existe esta clasificación para algún clasificación*/
                if(rs.next())
                {
                    /*Mensajea y continua*/
                    JOptionPane.showMessageDialog(null, "Ya esta esta clasificación: " + jTab.getValueAt(iSel[x], 1).toString() + " asignada a algún cliente, no se puede eliminar.", "Clasificación", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd)));                    
                    continue;
                }
            }
            catch(SQLException expnSQL)
            ...
            if(jTab.getValueAt(iSel[x], 1).toString().compareTo("SYS")==0)
            {
                /*Mensajea y continua*/
            ...
            try 
            {                
                sQ = "DELETE FROM clasemp WHERE cod = '" + jTab.getValueAt(iSel[x], 1).toString() + "'";                                            
                st = con.createStatement();
                st.executeUpdate(sQ);
             }
             catch(SQLException expnSQL) 
        ...
        /*Mensajea de éxito*/
        if(bMov)
            JOptionPane.showMessageDialog(null, "Clasficación(es) borrada(s) con éxito.", "Clasificaciones", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd))); 
    ...
    /*Cuando se presiona una tecla en el campo de edición de clasificación*/
    ...
    /*Cuando se gana el foco del teclado en el campo de edición de clasificación*/
    ...
    /*Cuando se presiona una tecla en el campo de descripción de clasificación*/
    ...
    /*Cuando se presiona una  tecla en la tabla de clasificación*/
    ...
    /*Cuando se gana el foco del teclado en el campo de edición de texto de descripción de clasificación*/
    ...
    private void jBNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBNewActionPerformed
        ...
        /*Si hay cadena vacia en el campo del código de la clasificación no puede continuar*/
        if(jTCod.getText().replace(" ", "").trim().compareTo("")==0)
        {
            ...
            /*Mensajea*/
            JOptionPane.showMessageDialog(null, "El código de la clasificación esta vacio.", "Campo vacio", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd)));
        ...
        /*Si hay cadena vacia en el campo de descripción de la clasificación no puede continuar*/
        if(jTDescrip.getText().compareTo("")==0)
        {
            /*Coloca el borde rojo*/                               
            ...
            /*Mensajea*/
            JOptionPane.showMessageDialog(null, "El campo de descripción de la clasificación esta vacio.", "Campo vacio", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd)));
        ...
        //Checa si el código de la clasificación ya existe en la base de datos               
        int iRes    = Star.iExistClasCli(con, jTCod.getText().replace(" ", "").trim());
        
        //Si hubo error entonces regresa
        if(iRes==-1)
            return;
        
        //Si la clasifiación existe entonces
        if(iRes==1)
        {
            //Cierra la base de datos
            if(Star.iCierrBas(con)==-1)
            ...
            /*Avisa al usuario*/
            JOptionPane.showMessageDialog(null, "El código de la clasificación: " + jTCod.getText().replace(" ", "").trim() + " ya existe.", "Registro Existente", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd))); 

            /*Pon el foco del teclado en el campo del código de la clasificación y regresa*/
            jTCod.grabFocus();               
            return;            
        }
                        
        /*Checa si la descripción de la clasificación ya existe en la base de datos*/        
        try
        {
            sQ = "SELECT descrip FROM clasemp WHERE descrip = '" + jTDescrip.getText() + "'";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
            /*Si hay datos*/
            if(rs.next())
            {
                ...
                /*Avisa al usuario*/
                JOptionPane.showMessageDialog(null, "La desripción: " + jTDescrip.getText() + " de la clasificación: " + jTCod.getText().replace(" ", "").trim() + " ya existe.", "Registro Existente", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd))); 
        ...
        Object[] op = {"Si","No"};
        iRes        = JOptionPane.showOptionDialog(this, "¿Seguro que están bien los datos?", "Agregar clasificación", JOptionPane.YES_NO_OPTION,  JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconDu)), op, op[0]);
        if(iRes==JOptionPane.NO_OPTION || iRes==JOptionPane.CLOSED_OPTION)
        {
            //Cierra la base de datos
            if(Star.iCierrBas(con)==-1)
        ...
        try 
        {            
            sQ = "INSERT INTO clasemp(cod,                                                                                              descrip,                                                    falt,       estac,                                          sucu,                                           nocaj) " + 
        ...
        /*Agrega el registro de la clasificación en la tabla*/
        DefaultTableModel te    = (DefaultTableModel)jTab.getModel();
        ...
        /*Pon el foco del teclado en el campo del código de la clasificación*/
        jTCod.grabFocus();
        
        
        /*Mensajea de éxito*/
        JOptionPane.showMessageDialog(null, "Clasificación: " + jTDescrip.getText() + " agregada con éxito.", "Exito al agregar", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd)));                    
    ...
    /*Función que carga nuevamente los elementos en la tabla*/
    private void vCargT()
    {
        ...
        /*Obtén de la base de datos las clasificacións y cargalas en la tabla*/
        vCargClas();
    ...
    private void jBBuscActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBBuscActionPerformed
        ...
        /*Borra todos los item en la tabla de clasificacións*/
        DefaultTableModel dm = (DefaultTableModel)jTab.getModel();
        dm.setRowCount(0);
        ...
        /*Obtiene de la base de datos todos las clasificacións*/        
        try
        {                  
            sQ = "SELECT * FROM clasemp WHERE cod LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR descrip LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR falt LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR fmod LIKE('%" + jTBusc.getText().replace(" ", "%") + "%')";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
            /*Si hay datos*/
            while(rs.next())
