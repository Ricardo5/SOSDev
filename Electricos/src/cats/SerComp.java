...
public class SerComp extends javax.swing.JFrame 
{
    ...
    public SerComp() 
    {
        ...
        this.setTitle("Series compras, Usuario: <" + Login.sUsrG + "> " + Login.sFLog);        
            ...
            public void propertyChange(PropertyChangeEvent event) 
            {
                ...
                if("tableCellEditor".equals(pro)) 
                {
                    ...
                    else
                    {
                        ...
                        {
                            /*Comprueba si el folio ya esta usado por medio de la serie en alguna compra*/        
                            try
                            {
                                sQ = "SELECT codcomp FROM comprs WHERE codcomp = '" + sSerOriG + sConsec + "'";                        
                                st = con.createStatement();
                                rs = st.executeQuery(sQ);
                                /*Si hay datos, entonces si existe esta ser para alguna compra*/
                                if(rs.next())
                                {
                                    ...
                                    /*Mensajea*/
                                    JOptionPane.showMessageDialog(null, "Este consecutivo ya fue asignado a alguna compra, no se puede modificar.", "Series", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd))); 
                        ...
                        {                                                        
                            sQ = "UPDATE consecs SET "
                                    ...
                                    + "WHERE ser        = '" + sSerOriG.replace("'", "''") + "' AND tip = 'COMP'";                                                
                            st = con.createStatement();
                            st.executeUpdate(sQ);
                         }
                         catch(SQLException expnSQL) 
    ...
    private void vCargSer()
    {
        ...
        try
        {
            sQ = "SELECT ser, descrip, consec FROM consecs WHERE tip = 'COMP'";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
            /*Si hay datos*/
            while(rs.next())
    ...
    private void initComponents() {
        ...
        jP1.add(jBDel, new org.netbeans.lib.awtextra.AbsoluteConstraints(490, 70, 100, 30));
        ...
        jP1.add(jBSal, new org.netbeans.lib.awtextra.AbsoluteConstraints(490, 100, 100, 30));
        ...
        jBNew.setText("Nuevo");
        jBNew.setToolTipText("Nuevo Serie (Ctrl+N)");
    ...
    private void jBDelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBDelActionPerformed
        ...
        if(jTab.getSelectedRow()==-1)
        {
            ...
            /*Coloca el foco del teclado en la tabla y regres*/
            jTab.grabFocus();            
            return;            
        }
        ...
        Object[] op = {"Si","No"};
        int iRes    = JOptionPane.showOptionDialog(this, "¿Seguro que quiere borrar la(s) ser(s)?", "Borrar Serie", JOptionPane.YES_NO_OPTION,  JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconDu)), op, op[0]);
        ...
        {
            /*Comprueba si esta ser existe en alguna compra*/        
            try
            {
                sQ = "SELECT ser FROM comprs WHERE ser = '" + jTab.getValueAt(iSel[x], 1).toString() + "' AND tip = 'COMP'";                        
                st = con.createStatement();
                rs = st.executeQuery(sQ);
                ...
                {
                    /*Mensajea y continua*/
                    JOptionPane.showMessageDialog(null, "Ya esta esta serie: " + jTab.getValueAt(iSel[x], 1).toString() + " asignada a alguna compra, no se puede eliminar.", "Series", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd)));                    
                    continue;
                }
            }
            catch(SQLException expnSQL)
            ...
            try 
            {                
                sQ = "DELETE FROM consecs WHERE ser = '" + jTab.getValueAt(iSel[x], 1).toString() + "' AND tip = 'COMP'";                                            
                st = con.createStatement();
                st.executeUpdate(sQ);
             }
             catch(SQLException expnSQL) 
    ...
    private void jTSerFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_jTSerFocusLost
        ...
        if(jTSer.getText().compareTo("")!=0)
            jTSer.setBorder(javax.swing.BorderFactory.createLineBorder(new Color(204,204,255)));
    ...
    private void jTConsecFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_jTConsecFocusLost
        ...
        if(jTConsec.getText().compareTo("")!=0)
            jTConsec.setBorder(javax.swing.BorderFactory.createLineBorder(new Color(204,204,255)));
    ...
    private void jBNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBNewActionPerformed
        ...
        if(jTConsec.getText().trim().compareTo("")==0)
        {
            /*Mensajea*/
        ...
        if(jTDescrip.getText().trim().compareTo("")==0)
        {
            ...
            /*Mensajea*/
            JOptionPane.showMessageDialog(null, "El campo de descripción de la ser esta vacio.", "Campo vacio", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd)));
        ...
        /*Si la serie a insertar es II entonces*/
        if(jTSer.getText().trim().toLowerCase().compareTo("ii")==0)
        {
            /*Coloca el borde rojo*/                               
            jTSer.setBorder(javax.swing.BorderFactory.createLineBorder(java.awt.Color.RED));
            
            /*Mensajea*/
            JOptionPane.showMessageDialog(null, "La serie no puede ser 'II' ni 'ii'.", "Campo vacio", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd)));
        ...
        /*Checa si la ser ya existe en la base de datos*/        
        try
        {
            sQ = "SELECT ser FROM consecs WHERE ser = '" + jTSer.getText().replace(" ", "").trim() + "' AND tip ='COMP'";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
        ...
        {            
            sQ = "INSERT INTO consecs( tip,         ser,                                                                                        Descrip,                                                                    consec,                                                 estac,                                          sucu,                                           nocaj) " + 
                             "VALUES(  'COMP', '" + jTSer.getText().replace(" ", "").trim().replace("'", "''").replace(",", "") + "', '" +      jTDescrip.getText().trim().replace("'", "''").replace(",", "") + "'," +     jTConsec.getText().trim().replace("'", "''") + ", '" +  Login.sUsrG.replace("'", "''") + "', '" +   Star.sSucu.replace("'", "''") + "', '" +  Star.sNoCaj.replace("'", "''") + "')";                    
            st = con.createStatement();
            st.executeUpdate(sQ);
         }
         catch(SQLException expnSQL) 
        ...
        DefaultTableModel te    = (DefaultTableModel)jTab.getModel();
        Object nu[]             = {iContFi, jTSer.getText().replace(" ", "").trim(), jTDescrip.getText().trim(), jTConsec.getText().trim()};
        te.addRow(nu);
    ...
    private void jBBuscActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBBuscActionPerformed
        ...
        try
        {                  
            sQ = "SELECT ser, descrip, consec FROM consecs WHERE tip = 'COMP' AND (ser LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR descrip LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR consec LIKE('%" + jTBusc.getText().replace(" ", "%") + "%'))";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
            /*Si hay datos*/
            while(rs.next())
    ...
    void vKeyPreEsc(java.awt.event.KeyEvent evt)
    {
        ...
        /*Si se presiona CTRL + N entonces presiona el botón de nuevo*/
            ...
            jBNew.doClick();
        /*Si se presiona F3 presiona el boton de búscar*/
        else if(evt.getKeyCode() == KeyEvent.VK_F3)
            jBBusc.doClick();            
        /*Else if se presiona Alt + F4 entonces presiona el botón de salir*/
        else if(evt.isAltDown() && evt.getKeyCode() == KeyEvent.VK_F4)
            jBSal.doClick();        
        /*Si se presiona F4 presioan el botón de mostrar todo*/
