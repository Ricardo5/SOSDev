...
import ptovta.Star;
import ptovta.Login;
import vis.VisSerGral;



/*Clase para controlar las series*/
public class SerPrev extends javax.swing.JFrame 
{
    ...
    public SerPrev() 
    {
        ...
        this.setTitle("Series remisiones, Usuario: <" + Login.sUsrG + "> " + Login.sFLog);        
...
//                            try
//                            {
//                                sQ = "SELECT noser FROM vtas WHERE noser = '" + sSerOriG + "' AND norefer = " + sConsec + " AND tipdoc = 'REM'";                        
...
//                        
                        ...
                        {                                                        
                            sQ = "UPDATE consecs SET "
                                    ...
                                    + "WHERE ser        = '" + sSerOriG.replace("'", "''") + "' AND tip = 'PREV'";                                                
                            st = con.createStatement();
                            st.executeUpdate(sQ);
                         }
                         catch(SQLException ex) 
    ...
    private void vCargSer()
    {
        ...
        try
        {
            sQ = "SELECT ser, descrip, consec FROM consecs WHERE tip = 'PREV'";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
            /*Si hay datos*/
            while(rs.next())
    ...
    private void initComponents() {
        ...
        jP1.add(jBTod, new org.netbeans.lib.awtextra.AbsoluteConstraints(365, 52, 125, 18));
    ...
    private void jBDelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBDelActionPerformed
        ...
        {
//            /*Comprueba si esta ser existe en alguna venta de remisión*/        
//            try
//            {
//                sQ = "SELECT noser FROM vtas WHERE noser = '" + jTab.getValueAt(iSel[x], 1).toString() + "' AND tipdoc = 'REM'";                        
//                st = con.createStatement();
//                rs = st.executeQuery(sQ);
...
//                {
//                    /*Mensajea y continua*/
//                    JOptionPane.showMessageDialog(null, "Ya esta esta serie: " + jTab.getValueAt(iSel[x], 1).toString() + " asignada a alguna venta de remisión, no se puede eliminar.", "Series", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd)));                    
//                    continue;
//                }
//
//            }/*Fin de try*/
...
//            } 
            ...
            try 
            {                
                sQ = "DELETE FROM consecs WHERE ser = '" + jTab.getValueAt(iSel[x], 1).toString() + "' AND tip = 'PREV'";                                            
                st = con.createStatement();
                st.executeUpdate(sQ);
             }
             catch(SQLException ex) 
    ...
    private void jBNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBNewActionPerformed
        ...
        try
        {
            sQ = "SELECT ser FROM consecs WHERE ser = '" + jTSer.getText().replace(" ", "").trim() + "' AND tip = 'PREV'";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
        ...
        {            
            sQ = "INSERT INTO consecs( tip,        ser,                                                                                         Descrip,                                                                        consec,                             estac,                                        sucu,                                          nocaj) " + 
                             "VALUES(  'PREV', '" + jTSer.getText().replace(" ", "").replace(",", "").replace("'", "''").trim() + "', '" +       jTDescrip.getText().trim().replace("'", "''").replace(",", "") + "'," +         sConsec.replace("'", "''") + ", '" +Login.sUsrG.replace("'", "''") + "', '" + Star.sSucu.replace("'", "''") + "', '" + Star.sNoCaj.replace("'", "''") + "')";                    
            st = con.createStatement();
            st.executeUpdate(sQ);
         }
         catch(SQLException ex) 
    ...
    private void jBBuscActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBBuscActionPerformed
        ...
        try
        {                  
            sQ = "SELECT ser, descrip, consec FROM consecs WHERE tip = 'PREV' AND (ser LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR descrip LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR consec LIKE('%" + jTBusc.getText().replace(" ", "%") + "%'))";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
            /*Si hay datos*/
            while(rs.next())
    ...
    private void jBTab1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBTab1ActionPerformed
        ...
        for(int x = 0; x < jTab.getRowCount(); x++)
        {
            ...
            Object nu[]             = {jTab.getValueAt(x, 0), jTab.getValueAt(x, 1), jTab.getValueAt(x, 2), jTab.getValueAt(x, 3)};
            te.addRow(nu);
        }
