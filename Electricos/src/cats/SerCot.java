...
/*Clase para controlar las series de las cots*/
public class SerCot extends javax.swing.JFrame 
{
    ...
    /*Declara los contadores globales*/
    ...
    public SerCot() 
    {
        ...
        this.setTitle("Series cotizaciones, Usuario: <" + Login.sUsrG + "> " + Login.sFLog);        
            ...
            public void propertyChange(PropertyChangeEvent event) 
            {
                ...
                if("tableCellEditor".equals(pro)) 
                {
                    ...
                    else
                    {
                        ...
                        {
                            /*Comprueba si el folio ya esta usado por medio de la serie en alguna cotización*/        
                            try
                            {
                                sQ = "SELECT codcot FROM cots WHERE codcot = '" + sSerOriG + sConsec + "'";                        
                                st = con.createStatement();
                                rs = st.executeQuery(sQ);
                                /*Si hay datos, entonces si existe este conscutivo en alguna cotización*/
                                if(rs.next())
                                {
                                    ...
                                    /*Mensajea*/
                                    JOptionPane.showMessageDialog(null, "Este consecutivo ya fue asignado a alguna cotización, no se puede modificar.", "Series", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd))); 
                        ...
                        {                                                        
                            sQ = "UPDATE consecs SET "
                                    ...
                                    + "WHERE ser        = '" + sSerOriG.replace("'", "''") + "' AND tip = 'COT'";                                                
                            st = con.createStatement();
                            st.executeUpdate(sQ);
                         }
                         catch(SQLException expnSQL) 
    ...
    private void vCargSer()
    {
        ...
        try
        {
            sQ = "SELECT ser, descrip, consec FROM consecs WHERE tip = 'COT'";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
            /*Si hay datos*/
            while(rs.next())
    ...
    private void jBDelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBDelActionPerformed
        ...
        {
            /*Comprueba si esta ser existe en alguna cotización*/        
            try
            {
                sQ = "SELECT ser FROM cots WHERE ser = '" + jTab.getValueAt(iSel[x], 1).toString() + "'";                        
                st = con.createStatement();
                rs = st.executeQuery(sQ);
                ...
                {
                    /*Mensajea y continua*/
                    JOptionPane.showMessageDialog(null, "Ya esta esta serie: " + jTab.getValueAt(iSel[x], 1).toString() + " asignada a alguna cotización, no se puede eliminar.", "Series", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd)));                    
                    continue;
                }
            }
            catch(SQLException expnSQL)
            ...
            try 
            {                
                sQ = "DELETE FROM consecs WHERE ser = '" + jTab.getValueAt(iSel[x], 1).toString() + "' AND tip = 'COT'";                                            
                st = con.createStatement();
                st.executeUpdate(sQ);
             }
             catch(SQLException expnSQL) 
    ...
    private void jBNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBNewActionPerformed
        ...
        /*Lee el folio de la serie que ingreso el usuario*/
        String sConsec;
        ...
        try
        {
            sQ = "SELECT ser FROM consecs WHERE ser = '" + jTSer.getText().replace(" ", "").trim() + "' AND tip = 'COT'";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
        ...
        {            
            sQ = "INSERT INTO consecs( tip,        ser,                                                                                         Descrip,                                                                    consec,                                 estac,                                          sucu,                                           nocaj) " + 
                             "VALUES(  'COT', '" + jTSer.getText().replace(" ", "").replace(",", "").replace("'", "''").trim() + "', '" +       jTDescrip.getText().trim().replace("'", "''").replace(",", "") + "'," +     sConsec.replace("'", "''") + ", '" +    Login.sUsrG.replace("'", "''") + "', '" +   Star.sSucu.replace("'", "''") + "', '" +  Star.sNoCaj.replace("'", "''") + "')";                    
            st = con.createStatement();
            st.executeUpdate(sQ);
         }
         catch(SQLException expnSQL) 
    ...
    private void jBBuscActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBBuscActionPerformed
        ...
        try
        {                  
            sQ = "SELECT ser, descrip, consec FROM consecs WHERE tip = 'COT' AND (ser LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR descrip LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR consec LIKE('%" + jTBusc.getText().replace(" ", "%") + "%'))";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
            /*Si hay datos*/
            while(rs.next())
