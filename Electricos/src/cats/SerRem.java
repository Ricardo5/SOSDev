...
public class SerRem extends javax.swing.JFrame 
{
                    ...
                    else
                    {
                        ...
                        {                                                        
                            sQ = "UPDATE consecs SET "
                                    ...
                                    + "WHERE ser        = '" + sSerOriG.replace("'", "''") + "' AND tip = 'REM'";                                                
                            st = con.createStatement();
                            st.executeUpdate(sQ);
                         }
                         catch(SQLException expnSQL) 
    ...
    private void vCargSer()
    {
        ...
        try
        {
            sQ = "SELECT ser, descrip, consec FROM consecs WHERE tip = 'REM'";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
            /*Si hay datos*/
            while(rs.next())
    ...
    private void jBDelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBDelActionPerformed
        ...
        for(int x = iSel.length - 1; x >= 0; x--)
        {
            ...
            try 
            {                
                sQ = "DELETE FROM consecs WHERE ser = '" + jTab.getValueAt(iSel[x], 1).toString() + "' AND tip = 'REM'";                                            
                st = con.createStatement();
                st.executeUpdate(sQ);
             }
             catch(SQLException expnSQL) 
    ...
    private void jBNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBNewActionPerformed
        ...
        try
        {
            sQ = "SELECT ser FROM consecs WHERE ser = '" + jTSer.getText().replace(" ", "").trim() + "' AND tip = 'REM'";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
        ...
        {            
            sQ = "INSERT INTO consecs( tip,        ser,                                                                                         Descrip,                                                                        consec,                             estac,                                        sucu,                                          nocaj) " + 
                             "VALUES(  'REM', '" + jTSer.getText().replace(" ", "").replace(",", "").replace("'", "''").trim() + "', '" +       jTDescrip.getText().trim().replace("'", "''").replace(",", "") + "'," +         sConsec.replace("'", "''") + ", '" +Login.sUsrG.replace("'", "''") + "', '" + Star.sSucu.replace("'", "''") + "', '" + Star.sNoCaj.replace("'", "''") + "')";                    
            st = con.createStatement();
            st.executeUpdate(sQ);
         }
         catch(SQLException expnSQL) 
