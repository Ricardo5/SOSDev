...
/*Clase para controlar las series de las órdenes de compra*/
public class SerOrd extends javax.swing.JFrame 
{
    ...
    public SerOrd() 
    {
        ...
        this.setTitle("Series órdenes compra, Usuario: <" + Login.sUsrG + "> " + Login.sFLog);        
            ...
            {                                
                ...
                if("tableCellEditor".equals(pro)) 
                {
                    ...
                    else
                    {
                        ...
                        /*Si lo que se modificó fue el consecutivo entonces*/
                        if(sConsec.compareTo(sConsecOriG)!=0)
                        {
                            /*Comprueba si el folio ya esta usado por medio de la serie en alguna órden de compra*/        
                            try
                            {
                                sQ = "SELECT noser FROM comprs WHERE codcomp = '" + sSerOriG + sConsec + "' AND tipdoc = 'ORD'";                        
                                st = con.createStatement();
                                rs = st.executeQuery(sQ);
                                /*Si hay datos, entonces si existe esta serie spara alguna compra*/
                                if(rs.next())
                                {
                                    ...
                                    /*Mensajea*/
                                    JOptionPane.showMessageDialog(null, "Este consecutivo ya fue asignado a alguna órden de compra, no se puede modificar.", "Series", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd))); 
                        ...
                        {                                                        
                            sQ = "UPDATE consecs SET "
                                    ...
                                    + "WHERE ser        = '" + sSerOriG.replace("'", "''") + "' AND tip = 'ORDC'";                                                
                            st = con.createStatement();
                            st.executeUpdate(sQ);
                         }
                         catch(SQLException expnSQL) 
    ...
    private void vCargSer()
    {
        ...
        try
        {
            sQ = "SELECT ser, descrip, consec FROM consecs WHERE tip = 'ORDC'";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
            /*Si hay datos*/
            while(rs.next())
    ...
    private void jBDelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBDelActionPerformed
        ...
        {
            /*Mensajea*/
            JOptionPane.showMessageDialog(null, "No has seleccionado una serie de la tabla para borrar.", "Borrar Serie", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd)));
        ...
        {
            /*Comprueba si esta serie existe en alguna compra*/        
            try
            {
                sQ = "SELECT ser FROM comprs WHERE ser = '" + jTab.getValueAt(iSel[x], 1).toString() + "' AND tip = 'ORDC'";                        
                st = con.createStatement();
                rs = st.executeQuery(sQ);
                ...
                {
                    /*Mensajea y continua*/
                    JOptionPane.showMessageDialog(null, "Ya esta esta serie: " + jTab.getValueAt(iSel[x], 1).toString() + " asignada a alguna orden de compra, no se puede eliminar.", "Series", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd)));                    
                    continue;
                }
            }
            catch(SQLException expnSQL)
            ...
            try 
            {                
                sQ = "DELETE FROM consecs WHERE ser = '" + jTab.getValueAt(iSel[x], 1).toString() + "' AND tip = 'ORDC'";                                            
                st = con.createStatement();
                st.executeUpdate(sQ);
             }
             catch(SQLException expnSQL) 
    ...
    private void jBNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBNewActionPerformed
        ...
        /*Si hay cadena vacia en el campo de la serie no puede continuar*/
        ...
        try
        {
            sQ = "SELECT ser FROM consecs WHERE ser = '" + jTSer.getText().replace(" ", "").trim() + "' AND tip ='ORDC'";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
        ...
        {            
            sQ = "INSERT INTO consecs( tip,          ser,                                                                                       Descrip,                                                                        consec,                                                 estac,                                          sucu,                                           nocaj) " + 
                             "VALUES(  'ORDC', '" +  jTSer.getText().replace(" ", "").replace(",", "").replace("'", "''").trim() + "', '" +     jTDescrip.getText().trim().replace("'", "''").replace(",", "") + "'," +         jTConsec.getText().replace("'", "''") + ", '" +         Login.sUsrG.replace("'", "''") + "', '" +   Star.sSucu.replace("'", "''") + "', '" +  Star.sNoCaj.replace("'", "''") + "')";                    
            st = con.createStatement();
            st.executeUpdate(sQ);
         }
         catch(SQLException expnSQL) 
        ...
        /*Agrega el registro de la serie en la tabla*/
        DefaultTableModel te    = (DefaultTableModel)jTab.getModel();
        Object nu[]             = {iContFi, jTSer.getText().replace(" ", "").trim(), jTDescrip.getText().trim(), jTConsec.getText()};
        te.addRow(nu);
    ...
    private void jBBuscActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBBuscActionPerformed
        ...
        try
        {                  
            sQ = "SELECT ser, descrip, consec FROM consecs WHERE tip = 'ORD' AND (ser LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR descrip LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR consec LIKE('%" + jTBusc.getText().replace(" ", "%") + "%'))";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
            /*Si hay datos*/
            while(rs.next())
