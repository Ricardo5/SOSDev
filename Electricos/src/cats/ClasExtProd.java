...
/*Clase para controlar las clasificaciones extras de los prods*/
public class ClasExtProd extends javax.swing.JFrame 
{
    ...
    {                
        ...
        /*Obtiene todas las clasificaciones de la base de datos y cargalas en la tabla*/
        vCargClas();
            ...
            public void propertyChange(PropertyChangeEvent event) 
            {
                ...
                if("tableCellEditor".equals(pro)) 
                {
                    ...
                    else
                    {
                        ...
                        try 
                        {                                                        
                            sQ = "UPDATE clasprov SET descrip = '" + sDescrip.replace("'", "''").replace(",", "") + "', fmod = now(), estac = '" + Login.sUsrG.replace("'", "''") + "', sucu = '" + Star.sSucu.replace("'", "''") + "', nocaj = '" + Star.sNoCaj.replace("'", "''") + "' WHERE cod = '" + sCodOriG.replace("'", "''") + "'";                                                
    ...
    private void initComponents() {
        ...
        jBNew.setText("Nuevo");
        jBNew.setToolTipText("Nueva Clasificación (Ctrl+N)");
    ...
    {        
        ...
        /*Trae las clasificaciones de la base de datos y cargalas en la tabla*/
        try
        {
            ...
            {
                /*Obtiene el código*/
                sClas                   = rs.getString("cod");
                
                /*Obtiene la descripción del peso*/
                sDescrip                = rs.getString("descrip");                                 
                
                /*Agregalo a la tabla*/
                Object nu[]             = {iContFi, sClas, sDescrip};
                te.addRow(nu);
    ...
    private void jBDelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBDelActionPerformed
        ...
        {
            /*Comprueba si esta clase existe en algún producto*/        
            try
            {
                sQ = "SELECT codext FROM prods WHERE codext = '" + jTab.getValueAt(iSel[x], 1).toString() + "'";                        
                st = con.createStatement();
                rs = st.executeQuery(sQ);
                /*Si hay datos, entonces si esta asociado este peso a algún producto*/
                if(rs.next())
                {
                    /*Mensajea y continua*/
                    JOptionPane.showMessageDialog(null, "Ya esta esta clasificación: " + jTab.getValueAt(iSel[x], 1).toString() + " asignado a algún producto, no se puede eliminar.", "Clasificación", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd)));                    
                    continue;
                }
            }
            catch(SQLException expnSQL)
            ...
            try 
            {                
                sQ = "DELETE FROM clasprod WHERE cod = '" + jTab.getValueAt(iSel[x], 1).toString() + "'";                                            
                st = con.createStatement();
                st.executeUpdate(sQ);
             }
             catch(SQLException expnSQL) 
            ...
            /*Inserta en log de clasificación extra de prods*/
            try 
            {            
                sQ = "INSERT INTO logclasext(cod,                                                                                           descrip,                                                                                    accio,              estac,                          sucu,                           nocaj,                      falt) " + 
                                "VALUES('" + jTab.getValueAt(iSel[x], 1).toString().replace("'", "''").replace(",", "") + "', '" +          jTab.getValueAt(iSel[x], 2).toString().replace("'", "''").replace(",", "") + "',            'BORRAR',     '" +   Login.sUsrG + "',   '" +    Star.sSucu + "',  '" +    Star.sNoCaj + "',     now())";                                
                st = con.createStatement();
                st.executeUpdate(sQ);
            }
            catch(SQLException expnSQL) 
    ...
    /*Cuando se gana el foco del teclado en el campo del código del peso*/
    ...
    /*Cuando se pierde el foco del teclado en el campo del código*/
    ...
    private void jBNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBNewActionPerformed
        ...
        //Si la clasificación existe entonces
        if(iRes==1)
        {
            //Cierra la base de datos
            if(Star.iCierrBas(con)==-1)
        ...
        try 
        {            
            sQ = "INSERT INTO clasprod(     cod,                                                                descrip,                                        falt,       estac,                                          sucu,                                           nocaj) " + 
        ...
        /*Inserta en log de clasificación de prods adicional*/
        try 
        {            
            sQ = "INSERT INTO logclasext(cod,                                                                       descrip,                                            accio,                  estac,                          sucu,                           nocaj,                      falt) " + 
    ...
    private void vCargT()
    {
        ...
        /*Obtiene todos las clasificaciones de la base de datos y cargalas en la tabla*/
        vCargClas();
    ...
    private void jBBuscActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBBuscActionPerformed
        ...
        /*Obtiene de la base de datos todas las clasificaciones*/        
        try
        {                  
            sQ = "SELECT * FROM clasprod WHERE cod LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR descrip LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR falt LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR fmod LIKE('%" + jTBusc.getText().replace(" ", "%") + "%')";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
            /*Si hay datos*/
            while(rs.next())
