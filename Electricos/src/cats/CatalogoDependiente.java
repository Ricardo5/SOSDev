...
public class CatalogoDependiente extends javax.swing.JFrame 
{
    ...
    private void jBNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBNewActionPerformed
        ...
        try 
        {            
            sQ = "INSERT INTO " + sTip + "(" + sTipCod + ",                                                                      descrip,                                        falt,           estac,                                          sucu,                                           nocaj, codZona) " + 
                       "VALUES('" + jTCod.getText().replace(" ", "").trim().replace("'", "''") + "','" +      jTDescrip.getText().replace("'", "''") + "',        now(), '" +     Login.sUsrG.replace("'", "''") + "','" +    Star.sSucu.replace("'", "''") + "','" +   Star.sNoCaj.replace("'", "''") + "','"+jCDatosDependientes.getSelectedItem().toString() + "')";                                           
            st = con.createStatement();
            st.executeUpdate(sQ);
         }
         catch(SQLException expnSQL) 
    ...
    private void jCDatosDependientesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jCDatosDependientesActionPerformed
        ...
        try
        {
            sQ = "SELECT descrip FROM zona WHERE cod = '" + jCDatosDependientes.getSelectedItem().toString().trim() + "'";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
