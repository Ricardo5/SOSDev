...
public class SerProv extends javax.swing.JFrame 
{
    ...
    public SerProv() 
    {
        ...
        this.setTitle("Series proveedores, Usuario: <" + Login.sUsrG + "> " + Login.sFLog);        
            ...
            public void propertyChange(PropertyChangeEvent event) 
            {
                ...
                if("tableCellEditor".equals(pro)) 
                {
                    ...
                    else
                    {
                        ...
                        {
                            /*Comprueba si el folio ya esta usado por medio de la serie en algún proveedor*/        
                            try
                            {
                                sQ = "SELECT prov FROM provs WHERE CONCAT_WS('', ser, prov)= '" + sSerOriG + sConsec +  "'";                        
                                ...
                                if(rs.next())
                                {
                                    ...
                                    /*Mensajea*/
                                    JOptionPane.showMessageDialog(null, "Este consecutivo ya fue asignado a algún proveedor, no se puede modificar.", "Series", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd))); 
                        ...
                        {                                                        
                            sQ = "UPDATE consecs SET "
                                    ...
                                    + "WHERE ser        = '" + sSerOriG.replace("'", "''") + "' AND tip = 'PROV'";                                                
                            st = con.createStatement();
                            st.executeUpdate(sQ);
                         }
                         catch(SQLException expnSQL) 
    ...
    private void vCargSer()
    {
        ...
        try
        {
            sQ = "SELECT ser, descrip, consec FROM consecs WHERE tip = 'PROV'";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
            /*Si hay datos*/
            while(rs.next())
    ...
    private void initComponents() {
        ...
        jP1.add(jBSal, new org.netbeans.lib.awtextra.AbsoluteConstraints(490, 100, 100, -1));
        ...
        jP1.add(jLabel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 10, 90, 20));
    ...
    private void jBDelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBDelActionPerformed
        ...
        {
            /*Comprueba si esta ser existe en algún proveedor*/        
            try
            {
                sQ = "SELECT ser FROM provs WHERE ser = '" + jTab.getValueAt(iSel[x], 1).toString() + "'";                        
                st = con.createStatement();
                rs = st.executeQuery(sQ);
                /*Si hay datos, entonces si existe esta ser asociado a algún proveedor*/
                if(rs.next())
                {
                    /*Mensajea y continua*/
                    JOptionPane.showMessageDialog(null, "Ya esta esta serie: " + jTab.getValueAt(iSel[x], 1).toString() + " asignada a algún proveedor, no se puede eliminar.", "Series", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconAd)));                    
                    continue;
                }
            }
            catch(SQLException expnSQL)
            ...
            try 
            {                
                sQ = "DELETE FROM consecs WHERE ser = '" + jTab.getValueAt(iSel[x], 1).toString() + "' AND tip = 'PROV'";                                            
                st = con.createStatement();
                st.executeUpdate(sQ);
             }
             catch(SQLException expnSQL) 
    ...
    private void jBNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBNewActionPerformed
        ...
        try
        {
            sQ = "SELECT ser FROM consecs WHERE ser = '" + jTSer.getText().replace(" ", "").trim() + "' AND tip = 'PROV'";
            st = con.createStatement();
            rs = st.executeQuery(sQ);
        ...
        {            
            sQ = "INSERT INTO consecs( tip,        ser,                                                                                             Descrip,                                                                    consec,                                 estac,                                          sucu,                                              nocaj) " + 
                             "VALUES(  'PROV', '" + jTSer.getText().replace(" ", "").replace(",", "").replace("'", "''").trim() + "', '" +          jTDescrip.getText().trim().replace("'", "''").replace(",", "") + "'," +     sConsec.replace("'", "''") + ", '" +    Login.sUsrG.replace("'", "''") + "', '" +   Star.sSucu.replace("'", "''") + "', '" +   Star.sNoCaj.replace("'", "''") + "')";                    
            st = con.createStatement();
            st.executeUpdate(sQ);
         }
         catch(SQLException expnSQL) 
    ...
    private void jBBuscActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBBuscActionPerformed
        ...
        try
        {                  
            sQ = "SELECT ser, descrip, consec FROM consecs WHERE tip = 'PROV' AND (ser LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR descrip LIKE('%" + jTBusc.getText().replace(" ", "%") + "%') OR consec LIKE('%" + jTBusc.getText().replace(" ", "%") + "%'))";            
            st = con.createStatement();
            rs = st.executeQuery(sQ);
            /*Si hay datos*/
            while(rs.next())
    ...
    /*Cuando se mueve el mouse la rueda del ratón en la forma*/
