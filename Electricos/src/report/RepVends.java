    ...
    private void jBAbrActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBAbrActionPerformed
        ...
        (new Thread()
        {
            ...
            {               
                ...
                try
                {
                    sQ = "SELECT SUM(vtas.TOT) AS total FROM vtas LEFT OUTER JOIN estacs ON estacs.ESTAC = vtas.ESTAC WHERE estacs.VEND = 1 AND vtas.ESTAD = 'CO' AND CASE WHEN '" + sCajFi + "' = '' THEN 1=1 ELSE vtas.NOCAJ = '" + sCajFi + "' END AND CASE WHEN '" + sSucuFi + "' = '' THEN 1=1 ELSE vtas.SUCU = '" + sSucuFi + "' END AND CASE WHEN '" + sEstaFi + "' = '' THEN 1=1 ELSE vtas.ESTAC = '" + sEstaFi + "' END AND CASE WHEN '" + sFDeFi + "'= '' THEN vtas.FEMI = vtas.FEMI ELSE DATE(vtas.FEMI) >= '" + sFDeFi + "' END AND CASE WHEN '" + sFAFi + "' = '' THEN vtas.FEMI = vtas.FEMI ELSE DATE(vtas.FEMI) <= '" + sFAFi + "' END AND CASE WHEN '" + sEmpFi + "' = '' THEN vtas.CODEMP = vtas.CODEMP ELSE vtas.CODEMP = '" + sEmpFi + "' END ";                    
                    st = con.createStatement();
                    rs = st.executeQuery(sQ);
                    /*Si hay datos entonces obtiene el resultado*/
                    if(rs.next())
                        sTotGlo      = rs.getString("total");                                   
                }
                catch(SQLException expnSQL)
                {
                    //Procesa el error y regresa
