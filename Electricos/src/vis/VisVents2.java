...
public class VisVents2 extends javax.swing.JFrame 
{
    ...
    {                
        ...
        jTab.getColumnModel().getColumn(4).setPreferredWidth(170);
        jTab.getColumnModel().getColumn(5).setPreferredWidth(400);
        jTab.getColumnModel().getColumn(14).setPreferredWidth(170);
        ...
        this.setTitle("Visor partidas de venta, Usuario: <" + Login.sUsrG + "> " + Login.sFLog);        
            ...
            public void propertyChange(PropertyChangeEvent event) 
            {
                ...
                if("tableCellEditor".equals(pr)) 
                {
                    ...
                    if(iContCellEd==1)
                    {
                        ...
                        sCantEntreOri   = jTab.getValueAt(jTab.getSelectedRow(), 3).toString();
                        sProdOri        = jTab.getValueAt(jTab.getSelectedRow(), 4).toString();
                        sDescripOri     = jTab.getValueAt(jTab.getSelectedRow(), 5).toString();
                        sDevsOri        = jTab.getValueAt(jTab.getSelectedRow(), 6).toString();
                        sKitOri         = jTab.getValueAt(jTab.getSelectedRow(), 7).toString();
                        sUnidOri        = jTab.getValueAt(jTab.getSelectedRow(), 8).toString();
                        sMonOri         = jTab.getValueAt(jTab.getSelectedRow(), 9).toString();
                        sPreOri         = jTab.getValueAt(jTab.getSelectedRow(), 10).toString();
                        sDescOri        = jTab.getValueAt(jTab.getSelectedRow(), 11).toString();
                        sImpoOri        = jTab.getValueAt(jTab.getSelectedRow(), 12).toString();
                        sImpueOri       = jTab.getValueAt(jTab.getSelectedRow(), 13).toString();
                        sFechOri        = jTab.getValueAt(jTab.getSelectedRow(), 14).toString();
                        ...
                        sCaduOri        = jTab.getValueAt(jTab.getSelectedRow(), 19).toString();                        
                        sBackOri        = jTab.getValueAt(jTab.getSelectedRow(), 20).toString();                                                
                        sSerOri         = jTab.getValueAt(jTab.getSelectedRow(), 21).toString();
                        sComenOri       = jTab.getValueAt(jTab.getSelectedRow(), 22).toString();
                    ...
                    else
                    {
                        ...
                        jTab.setValueAt(sCantEntreOri,  jTab.getSelectedRow(), 3);
                        jTab.setValueAt(sProdOri,       jTab.getSelectedRow(), 4);                        
                        jTab.setValueAt(sDescripOri,    jTab.getSelectedRow(), 5);                        
                        jTab.setValueAt(sDevsOri,       jTab.getSelectedRow(), 6);                        
                        jTab.setValueAt(sKitOri,        jTab.getSelectedRow(), 7);                        
                        jTab.setValueAt(sUnidOri,       jTab.getSelectedRow(), 8);                        
                        jTab.setValueAt(sMonOri,        jTab.getSelectedRow(), 9);                        
                        jTab.setValueAt(sPreOri,        jTab.getSelectedRow(), 10);                        
                        jTab.setValueAt(sDescOri,       jTab.getSelectedRow(), 11);                        
                        jTab.setValueAt(sImpoOri,       jTab.getSelectedRow(), 12);                        
                        jTab.setValueAt(sImpueOri,      jTab.getSelectedRow(), 13);                        
                        jTab.setValueAt(sFechOri,       jTab.getSelectedRow(), 14);                        
                        ...
                        jTab.setValueAt(sCaduOri,       jTab.getSelectedRow(), 19);                        
                        jTab.setValueAt(sBackOri,       jTab.getSelectedRow(), 20);                                                                        
                        jTab.setValueAt(sSerOri,        jTab.getSelectedRow(), 21);
                        jTab.setValueAt(sComenOri,      jTab.getSelectedRow(), 22);
        ...
        /*Si la venta no es nula entonces carga todos los datos de esa venta en la tabla*/
        if(sVta!=null)
            vCargP(sVta);            
    ...
    private void initComponents() {
        ...
        jTab.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "No.", "Venta", "Qty", "Entregados", "Producto", "Descripción", "Devueltos", "Kit", "Unidad", "Moneda", "Precio", "Descuento", "Importe", "Impuesto", "Fecha", "Talla", "Color", "Lote", "Pedimento", "Caducidad", "Backorder", "Serie Producto", "Comentario Serie"
            }
        ) {
    ...
    /*Carga las partidas de esa venta en la tabla*/
    private void vCargP(String sVta)
    {        
	    ...
	    /*Mensajea y regresa*/    
            JOptionPane.showMessageDialog(null, this.getClass().getName() + " Error al abrir la base de datos en jButtonSalirActionPerformed() por " + e.getMessage(), "Error al abrir base de datos", JOptionPane.ERROR_MESSAGE, new ImageIcon(getClass().getResource(Star.sRutIconEr))); 
            return;
        }
        ...
        try
        {
            ...
            if(rs.next())          
            {
                ...
                Object nu[]             = {iContFi, sVta, rs.getString("cant"), rs.getString("cantentre"), rs.getString("prod"), rs.getString("descrip"), rs.getString("devs"), rs.getString("eskit"), rs.getString("unid"), rs.getString("mon"), sPre, rs.getString("descu"), sImpo, rs.getString("impue"), rs.getString("falt"), rs.getString("tall"), rs.getString("colo"), rs.getString("lot"), rs.getString("pedimen"), rs.getString("fcadu"), rs.getString("fentre"), rs.getString("serprod"), rs.getString("comenser")};
                te.addRow(nu);                      
    ...
    private void formWindowStateChanged(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowStateChanged
        ...
        this.setExtendedState(this.getExtendedState() | VisVents2.MAXIMIZED_BOTH);
